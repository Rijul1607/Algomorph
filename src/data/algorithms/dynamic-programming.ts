...
function knapsack(values, weights, capacity) {
  ...
  generateSteps: (input: { values: number[], weights: number[], capacity: number }) => {
    ...
    const values = Array.isArray(input?.values) ? input.values : [60, 100, 120];
    const weights = Array.isArray(input?.weights) ? input.weights : [10, 20, 30];
    const capacity = typeof input?.capacity === "number" ? input.capacity : 50;
